...
#' @examples
#' ex_pop2names <- c("apple", "banana", "cucumber")
...
pop2_colnames <- function(popnames, popool_fst) {
	...
	popool_fst2_test <- data.frame(apply(popool_fst2, 2, function(x) as.numeric(gsub('.*=', '', x))))
	pop2_name <- data.frame(t(combn(popnames,2)))
	pop2_list <- paste(pop2_name$X1, pop2_name$X2, sep=':')
	colnames(popool_fst2_test) <- pop2_list
	file_path <- getwd()
	write.table(popool_fst2_test, paste(file_path, '/popool_fst_df', sep=''), row.names=FALSE, col.names=TRUE, quote=FALSE)
