...
#' @return data.frame amel_colnames_df chrom_pos
#' @examples
...
amel_colnames <- function(popnames, popool_fst) {
	...
	file_path <- getwd()
	write.table(popool_fst2, paste(file_path, '/amel_colnames_df', sep=''), row.names=FALSE, col.names=TRUE, quote=FALSE)
	write.table(chrom_pos, paste(file_path, '/chrom_pos_df', sep=''), row.names=FALSE, col.names=TRUE, quote=FALSE)
	if(file.exists("amel_colnames_df")==TRUE){print("fst table created")} 
}
