...
#' @param popnames text file with sample names in column 1 and population categories in columns 2:# (FORMERLY ID)
...
hbpop_gendiv <- function(geneouts, popnames, imm_genes, GB_GBold, GB_to_NCBI) {
  ##=== STARTUP ===##
  ...
  merge(popnames, merge(my.ncbi, df, by="product"))
  ...
  temp2 <- t.test(temp$NS~temp$Field1)
  temp2$p.value
  p <- ggplot(temp, aes(x=Field1, y=NS, fill=Field1)) + geom_boxplot(outlier.color="black", notch=FALSE) + labs(x="Management", y="NS") + theme_classic() + theme(legend.position="none", axis.title.x=element_text(color="black", size=12), axis.text.y=element_text(color="black", size=12)) + ylab("𝜋N/𝜋S") + xlab("Management Practice") + ggtitle("Dscam")
