...
#  as.numeric(gsub('.*_', '', x))
#}
#test <- df %>% mutate(across(6:ncol(df), rm_chars))
...
```{r}
mean <- colMeans(df1)
mean.n0 <- apply(df1,2,function(x) mean(x[x>0]))
...
df_means %>% mutate(
  Field1 = gsub('[.].*','', df_means$stock), Field2 = gsub('.*[.]','', df_means$stock))
#.keep="unused" ?

df_hm <- subset(df_means, select=-c(3))
